...
class QwenTokenizerFastPPLCalc(object):
    ...
    def get_bbs_ll(self, input_ids, ll):
        ...
        for idx, token in enumerate(tokenized_tokens):
            ...
            if len(token.encode("utf-8")) == 1:
                ...
                byte_list = [
                        self.byte_encoder[b] for b in token.lstrip('Ġ').encode("utf-8")
                ]
    ...
    def forward_calc_ppl(self, text):
        doc_max_len = 512
        tokenized = self.base_tokenizer(text, max_length=doc_max_len, padding=True, truncation=True, return_tensors='pt')
        ...
        #outputs = self.last_token_pool(outputs.last_hidden_state, tokenized['attention_mask'])
